...
sgit.commit_bypath = commit_bypath = (function(path_to_file, message, callback) {
	...
	if (repo_path === 'undefined') {
        ...
        sgitNative.commit_bypath(repo_path,file_path,message,function(err) {
                callback(err);
        });
	return 0;
});
...
sgit.log = log = (function(repo_path, callback) {
        ...
        sgitNative.log(repo_path, function(err,log) {
